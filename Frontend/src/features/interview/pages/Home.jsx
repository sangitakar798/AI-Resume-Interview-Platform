...
const Home = () => {
    ...
    const navigate = useNavigate()
      ...
      const handleGenerateReport = async () => {
        ...
        if (!selectedResume && !selfDescription.trim()) {
            setError("Upload a resume or provide a self-description.")
            return
        }
 ...
 const handleResumeChange = (event) => {
        const selectedResume = event.target.files?.[0] ?? null
        if (!selectedResume) {
            setResumeFile(null)
            return
        }

        if (!/\.pdf$/i.test(selectedResume.name)) {
        ...
        if (selectedResume.size > 3 * 1024 * 1024) {
    ...
    if (loading) {
        return <LoadingScreen label="Generating your interview plan..." />
    }
    ...
    return (
        <div className='home-page'>
                    ...
                    <div className='panel panel--right'>
                        ...
                        <div className='upload-section'>
                            ...
                            <label className={`dropzone ${resumeFile ? 'dropzone--selected' : ''}`} htmlFor='resume'>
                                ...
                                <p className='dropzone__title'>{resumeFile ? 'Resume selected' : 'Click to upload or drag & drop'}</p>
                            ...
                            {resumeFile && <button type='button' className='clear-resume' onClick={clearResume}>Remove resume</button>}
                        </div>
            ...
            <section className='workspace-links'>
                <button onClick={() => navigate('/applications')}><strong>Application Tracker</strong><span>Track companies, roles, deadlines and notes.</span></button>
            </section>
